...
def tokenise(data):
	...
	for i in data:
		...
		for j in range(len(chars) - 2):
			chars[j] = chars[j] + "_" + chars[j+1] + "_" + chars[j+2]
		chars = chars[:-1]
		a = [chars,i[1]]
		tok.append(a)
	return tok
...
def get_trigram(tok_data):
	trigram={}
	for i in tok_data:
		for j in range(len(i[0]) - 2):
			a = str(i[0][j]) + "_" + str(i[0][j+1]) + "_" + str(i[0][j+2])
			if a not in trigram:
				trigram[a] = 1
			else:
				trigram[a] +=1
...
print tok_data[0]
print "get trigram"
trigram = get_trigram(tok_data)
print "sorting"
res =[]
res = sorted(trigram, key=trigram.get, reverse=True)
res2=[]
for i in range(len(res)):
    res2.append(trigram[res[i]])
