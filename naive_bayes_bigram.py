...
def get_bigram(tok_data):
	bigram={}
	for i in tok_data:
		for j in range(len(i[0]) - 1):
			a = str(i[0][j]) + "_" + str(i[0][j+1])
			if a not in bigram:
				bigram[a] = 1
			else:
				bigram[a] +=1
...
print tok_data[0]
print "get bigram"
bigram = get_bigram(tok_data)
print "sorting"
res =[]
res = sorted(bigram, key=bigram.get, reverse=True)
res2=[]
for i in range(len(res)):
    res2.append(bigram[res[i]])
